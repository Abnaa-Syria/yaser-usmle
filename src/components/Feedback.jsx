  ...
  return (
    <div className="flex items-center gap-0.5" aria-hidden>
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          key={i}
          className={`${size} ${i <= v ? "fill-yu-blue-500-normal text-yu-blue-500-normal" : "fill-none text-slate-300"}`}
        />
      ))}
    </div>
  );
        ...
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:items-start lg:gap-16">
          <div className="text-start">
            <h2 className="max-w-md text-3xl font-bold leading-snug text-slate-900 md:text-4xl">
              ...
              <span className="rounded bg-yu-blue-500-normal px-1 text-slate-900">
            ...
            <Link
              to="/explore"
              className="mt-7 inline-block rounded-lg bg-yu-blue-700 px-7 py-3 text-sm font-semibold text-white transition hover:bg-yu-blue-600"
            >
              {t("feedback.button")}
            </Link>
          </div>
            ...
            {!reviewsLoading && !hasReviews ? (
              <div className="space-y-4 rounded-2xl border border-slate-200 bg-slate-50/80 p-8 text-start shadow-sm">
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  <Link
                    to="/explore"
                    className="inline-flex rounded-xl bg-yu-blue-700 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-yu-blue-600"
                  >
                    {t("feedback.empty.ctaExplore")}
                  </Link>
                  <Link
                    to="/signup"
                    className="inline-flex rounded-xl border border-slate-300 bg-white px-5 py-2.5 text-sm font-semibold text-slate-800 transition hover:border-yu-blue-700 hover:text-yu-blue-700"
                  >
            ...
            {hasReviews ? (
              <div className="space-y-6">
                ...
                <div className="flex flex-col gap-4 rounded-xl border border-slate-100 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <Link
                    to="/explore"
                    className="inline-flex shrink-0 justify-center rounded-xl bg-yu-blue-700 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-yu-blue-600"
                  >
                ...
                {avgRating > 0 ? (
                  <div className="rounded-xl bg-yu-blue-700 p-4 text-white shadow-md">
