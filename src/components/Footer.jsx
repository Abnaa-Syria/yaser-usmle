...
function SocialIcon({ label, Icon, href }) {
  ...
  const baseClass =
    "inline-flex h-9 w-9 items-center justify-center rounded-lg border border-white/10 bg-white/5 text-slate-400 transition hover:border-[var(--yu-blue-400)]/50 hover:bg-[var(--yu-blue-700)]/10 hover:text-[var(--yu-blue-300)]";
  if (href) {
    return (
...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-[var(--yu-blue-900)] bg-[var(--yu-blue-950)] text-slate-300">
      {/* USMLE learning areas strip */}
      <div className="border-b border-white/5 bg-[var(--yu-blue-900)]/50">
        <div className="mx-auto max-w-7xl px-4 py-5 md:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {DISCIPLINE_KEYS.map((key) => (
              <span
                key={key}
                className="rounded-full border border-white/10 bg-white/5 px-3.5 py-1.5 text-xs font-semibold text-slate-400 transition hover:border-[var(--yu-blue-400)]/30 hover:text-[var(--yu-blue-300)]"
              >
      ...
      <div className="mx-auto max-w-7xl px-4 pb-8 pt-12 md:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-5 lg:gap-8">
          <div className="lg:col-span-2">
            ...
            <div className="mt-5 space-y-2 text-sm text-slate-400">
              {site.contactEmail ? (
                <a href={mailto} className="inline-flex items-center gap-2 transition hover:text-[var(--yu-blue-300)]">
                  <Mail className="h-4 w-4 shrink-0 text-[var(--yu-blue-400)]" aria-hidden />
                  {site.contactEmail}
                </a>
              ) : null}
                ...
                {col.links.map((item) => (
                  <li key={`${col.title}-${item.label}`}>
                    {"href" in item ? (
                      <a
                        href={item.href}
                        className="text-sm text-slate-400 transition hover:text-[var(--yu-blue-300)]"
                      >
                        {item.label}
                      </a>
                    ) : (
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-3 border-t border-white/10 pt-6 md:flex-row">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 text-xs text-slate-500">
            <Link to="/terms" className="transition hover:text-[var(--yu-blue-300)]">
              {t("footer.teaching.terms")}
            </Link>
            <Link to="/privacy" className="transition hover:text-[var(--yu-blue-300)]">
              {t("footer.legal.privacy", { defaultValue: isRtl ? "الخصوصية" : "Privacy" })}
            </Link>
            <Link to="/contact" className="transition hover:text-[var(--yu-blue-300)]">
