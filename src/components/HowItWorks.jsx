      ...
      <div className="relative mx-auto max-w-[1320px] px-4 md:px-6 lg:px-8">
        ...
        >
          <p className="mb-4 inline-flex items-center gap-2 rounded-full border border-blue-100 bg-white px-3.5 py-2 text-[11px] font-extrabold text-blue-700 shadow-sm">
            ...
            {t("howItWorks.eyebrow", { defaultValue: isRtl ? "تجربة تعليمية متكاملة" : "A complete learning experience" })}
          </p>
          <h2 className="text-3xl font-black leading-[1.18] tracking-[-0.04em] text-slate-950 sm:text-4xl lg:text-5xl">
            {t("howItWorks.titleStart", { defaultValue: isRtl ? "رحلتك التعليمية" : "Your learning journey" })}{" "}
            <span className="bg-gradient-to-l from-blue-700 to-cyan-500 bg-clip-text text-transparent">
              {t("howItWorks.titleAccent", { defaultValue: isRtl ? "من الفهم إلى الثقة" : "from understanding to confidence" })}
            </span>
          </h2>
        ...
        <div className="mt-14 overflow-hidden rounded-[2.25rem] border border-slate-200/80 bg-white p-3 shadow-[0_25px_75px_rgba(15,23,42,.08)] sm:p-5 lg:p-7">
            ...
            {STEPS.map((stepKey, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`pointer-events-none absolute end-5 top-3 select-none text-[5.5rem] font-black leading-none tracking-[-0.09em] sm:end-7 sm:text-[6.5rem] ${tone.number}`} aria-hidden>{NUMS[idx]}</span>
          ...
          <div className="mt-4 flex flex-col items-center justify-between gap-5 rounded-[1.75rem] bg-[#081a39] px-6 py-6 text-white sm:flex-row sm:px-8">
            <div className="flex items-center gap-4">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white/10 text-cyan-200"><BookOpen className="h-5 w-5" aria-hidden /></div>
              <div><p className="text-sm font-black sm:text-base">{isRtl ? "كل أدوات تعلمك في مساحة واحدة" : "Every learning tool in one focused space"}</p><p className="mt-1 text-[10px] font-semibold text-blue-200/70">{isRtl ? "دروس، اختبارات، فلاش كاردز، وخطة مذاكرة متكاملة" : "Lessons, assessments, flashcards, and an integrated study plan"}</p></div>
            </div>
            <a href="#courses" className="group inline-flex shrink-0 items-center gap-2 rounded-xl bg-white px-4 py-3 text-xs font-extrabold text-[#081a39] transition hover:bg-blue-50">
