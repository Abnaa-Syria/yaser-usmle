function DataTable({ columns = [], rows = [], rowKey = "id", emptyNode = null, pagination = null }) {
  if (!rows.length && emptyNode) return emptyNode;

  return (
    <div className="overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800 dark:bg-[#1E293B] dark:shadow-2xl">
      ...
      {pagination ? (
        <div className="border-t border-slate-200/80 bg-slate-50/60 px-4 py-3 dark:border-white/8 dark:bg-[#0F172A]/50">
          {pagination}
        </div>
      ) : null}
    </div>
