...
export default function NotificationBell({ variant = "dashboard" }) {
  ...
  const buttonClass =
    variant === "dashboard"
      ...
      : "relative rounded-full p-2 text-slate-600 transition hover:bg-slate-100 hover:text-yu-blue-700";
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {unread > 0 ? (
          <span className="absolute end-1 top-1 flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-yu-blue-700 px-0.5 text-[10px] font-bold text-white ring-2 ring-white dark:ring-pioneer-dark-card">
            {unread > 9 ? "9+" : unread}
          </span>
        ) : null}
      </button>
      ...
      {open ? (
        <div className="absolute end-0 z-50 mt-3 w-[min(20rem,calc(100vw-1.5rem))] max-w-[calc(100vw-1.5rem)] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-md sm:mt-4 sm:w-80 sm:max-w-none dark:border-slate-800 dark:bg-[#1E293B] dark:shadow-2xl dark:shadow-black/50">
          <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3 dark:border-slate-700">
            ...
            <button
              ...
              className="text-[10px] font-bold text-yu-blue-700 hover:underline disabled:opacity-50"
            >
            ...
            {list.slice(0, 20).map((n) => (
              <button
                ...
                className={`block w-full border-b border-slate-100 px-4 py-3 text-start transition last:border-0 dark:border-slate-700 ${
                  n.isRead ? "text-slate-600 dark:text-slate-400" : "bg-yu-blue-100/20 dark:bg-yu-blue-700/10"
                }`}
              >
