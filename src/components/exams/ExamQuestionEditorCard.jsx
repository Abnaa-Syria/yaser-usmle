      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {type === "MULTIPLE_CHOICE" ? (
          <button
            ...
            className="inline-flex items-center gap-1 text-[10px] font-bold text-[var(--yu-blue-700)] disabled:opacity-40"
          >
        ...
        {rows.map((opt, idx) => {
          ...
          return (
            ...
            >
              ...
              {type === "MULTIPLE_CHOICE" ? (
                <>
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-[var(--yu-blue-700)]/10 text-[10px] font-bold text-[var(--yu-blue-700)]">
                    {letter}
                  </span>
                  <input
                    type="text"
  ...
  return (
    <div
      className={`overflow-hidden rounded-2xl border bg-white shadow-sm transition-all dark:bg-[#1A1A22] ${
        dirty ? "border-[var(--yu-blue-700)]/40 ring-1 ring-[var(--yu-blue-700)]/20" : "border-slate-200 dark:border-white/10"
      }`}
    >
      {/* Header */}
      <div className="flex items-center justify-between gap-2 border-b border-slate-100 px-4 py-3 dark:border-white/5">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[var(--yu-blue-700)] text-xs font-bold text-white">
            {index + 1}
          </span>
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          {dirty ? (
            <button
              ...
              onClick={handleSave}
              disabled={isSaving}
              className="inline-flex items-center gap-1 rounded-lg bg-[var(--yu-blue-700)] px-3 py-1.5 text-[10px] font-bold text-white hover:bg-[var(--yu-blue-600)] disabled:opacity-60"
            >
      ...
      <div className={`space-y-4 ${compact ? "p-3" : "p-4"}`}>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div>
            ...
            <input
              value={explanation}
              onChange={(e) => mark(setExplanation)(e.target.value)}
              className={INPUT}
              placeholder={label("explanationPlaceholder", "Shown after grading")}
            />
          </div>
        </div>
