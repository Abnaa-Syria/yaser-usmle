...
function Sidebar({ sections, isMobileOpen, setIsMobileOpen }: SidebarProps) {
  ...
  return (
    <aside
      className={`${widthClass} h-screen shrink-0 border-e border-slate-100 bg-white transition-transform duration-300 ease-in-out dark:border-white/6 dark:bg-[#0F1E38] max-lg:fixed max-lg:inset-y-0 max-lg:start-0 max-lg:z-[100] max-lg:shadow-2xl max-lg:max-w-[min(16rem,85vw)] lg:max-w-none lg:sticky lg:top-0 lg:z-30 lg:self-start lg:shadow-none lg:!translate-x-0 ${isMobileOpen
    ...
    >
      ...
      >
        <Link
          to="/"
          className={`overflow-hidden whitespace-nowrap transition hover:opacity-80 ${isCollapsed ? "lg:hidden" : ""}`}
          title={t("sidebarNav.items.websiteHome", { defaultValue: "Homepage" })}
        >
          <BrandLogo variant="primary" className="h-9 w-auto dark:hidden" />
          <BrandLogo variant="light" className="hidden h-9 w-auto dark:block" />
        </Link>
