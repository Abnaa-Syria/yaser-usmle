...
export default function InstructorCard({ instructor, isRtl }) {
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-3xl border border-slate-200/60 bg-white transition-all duration-300 hover:-translate-y-1.5 hover:border-[var(--yu-blue-700)]/30 hover:shadow-[0_20px_50px_rgba(238,124,17,0.08)] dark:border-slate-800 dark:bg-[#1E293B]">
      
      {/* Decorative top accent */}
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-orange-400 via-[var(--yu-blue-700)] to-amber-500 opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
      ...
      <div className="flex flex-1 flex-col p-6 pt-4">
        ...
        <Link 
          to={`/instructors/${instructor.id}`} 
          className="text-lg font-bold text-slate-850 dark:text-white transition hover:text-[var(--yu-blue-700)]"
        >
          {instructor.fullName}
        </Link>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          {instructor.experience != null && instructor.experience > 0 && (
            <span className="inline-flex items-center gap-1.5 rounded-lg bg-orange-50 px-2.5 py-1 text-[11px] font-semibold text-[var(--yu-blue-700)] dark:bg-orange-950/20">
        ...
        <p className="mt-4 line-clamp-3 flex-1 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
          {instructor.bio?.trim() || t("publicInstructors.noBio", { defaultValue: "Expert USMLE Step 1 medical instructor on Yaser USMLE." })}
        </p>
        ...
        <div className="mt-6 flex flex-col gap-2.5 sm:flex-row">
          <Link
            to={`/instructors/${instructor.id}`}
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-slate-200/80 bg-white px-4 py-2.5 text-xs font-bold text-slate-650 transition hover:border-[var(--yu-blue-700)] hover:text-[var(--yu-blue-700)] dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:border-[var(--yu-blue-700)]"
          >
            {t("publicInstructors.viewProfile", { defaultValue: "View Profile" })}
          </Link>
          <Link
            to={`/instructors/${instructor.id}#book`}
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-[var(--yu-blue-700)] px-4 py-2.5 text-xs font-black text-white shadow-md shadow-orange-500/10 transition hover:bg-orange-600 active:scale-[0.98]"
          >
            <CalendarClock className="h-3.5 w-3.5" />
            {t("publicInstructors.bookSession", { defaultValue: isRtl ? "طلب جلسة فردية" : "Request 1-on-1" })}
