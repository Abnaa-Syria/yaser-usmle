...
function UnitBlock({ unit, exams, defaultOpen, t }) {
          ...
          {sections.map((section) => (
            <div key={section.id} className="space-y-0.5">
              ...
              {(section.lessons || []).map((lesson) => {
                ...
                const meta = lesson.durationSeconds ? formatDuration(lesson.durationSeconds) : null;

                return (
                  <div key={lesson.id} className="space-y-0.5">
                    <OutlineRow
                      icon={Play}
                      iconClass="bg-yu-blue-100 text-yu-blue-700"
                      title={lesson.title}
                      meta={meta}
                      locked
                    />
...
export default function PublicCourseCurriculum({ units = [], exams = [] }) {
        ...
        <div className="space-y-3">
          ...
          {hiddenCount > 0 ? (
            <button
              ...
              className="w-full rounded-xl border border-dashed border-slate-200 py-2.5 text-sm font-semibold text-yu-blue-700 transition hover:border-yu-blue-700 hover:bg-yu-blue-100/30"
            >
              {showAll
