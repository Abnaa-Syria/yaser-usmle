...
export async function postLessonAccess(
  lessonId: string,
  courseId: string,
  watchPercentage = 0,
  extras: { lastWatchedPosition?: number; timeSpentDelta?: number } = {}
) {
  const res = await client.post(endpoints.student.progressLessonAccess(lessonId), {
    courseId,
    watchPercentage,
    ...extras,
  });
