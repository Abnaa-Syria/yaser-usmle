...
export default function CourseDetails() {
  ...
  const seoTitle = course ? (isRtl ? course.titleAr || course.title : course.title) : t("courses.detailTitle", { defaultValue: "Course details" });
  const seoDescription = course
    ? (isRtl
        ? course.shortDescriptionAr || course.descriptionAr || course.description
        : course.shortDescription || course.description) || t("courses.detailDescription", { defaultValue: "Explore Yaser USMLE course content, access options, and learning resources." })
  ...
  if (isError || !course) {
    return (
      <div className="min-h-screen bg-slate-50 py-16 text-center">
        ...
        <div className="mt-6">
          <Link to="/explore" className="text-sm font-semibold text-yu-blue-700">
    ...
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50/80 to-white py-10 md:py-14">
      <div className="mx-auto max-w-7xl px-4 md:px-6 lg:px-8">
        <nav className="mb-6 flex items-center gap-1.5 text-sm text-slate-500">
          <Link to="/explore" className="transition hover:text-[var(--yu-blue-700)]">
            {t("courseDetails.breadcrumb.explore")}
          </Link>
          ...
          <main className="space-y-8 lg:col-span-2">
            <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white p-6 shadow-sm md:p-8">
              {course.category?.name ? (
                <span className="inline-block rounded-full bg-yu-blue-100 px-3 py-1 text-xs font-bold text-yu-blue-700">
                  {course.category.name}
                </span>
              ) : null}
              ...
              {course.description ? (
                <div
                  className="course-rich-html mt-3 space-y-3 text-base leading-relaxed text-slate-600 [&_a]:font-semibold [&_a]:text-yu-blue-700 [&_a]:underline [&_img]:mx-auto [&_img]:my-4 [&_img]:h-auto [&_img]:max-w-full [&_img]:rounded-xl [&_p]:mb-2 [&_strong]:text-slate-800"
                  dangerouslySetInnerHTML={{ __html: sanitizeRichHtml(course.description) }}
                />
              ) : (
                <p className="mt-3 text-base leading-relaxed text-slate-600">—</p>
              )}
            ...
            {instructorForCard ? (
              <section className="overflow-hidden rounded-2xl border border-slate-100 bg-white p-6 shadow-sm md:p-8">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                  <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-yu-blue-700 text-xl font-extrabold text-white shadow">
                      ...
                      <div className="mt-4 flex flex-wrap gap-3">
                        {(platformFeatures.publicInstructorCatalog || platformFeatures.privateBooking) ? (
                          <Link
                            ...
                            className="inline-flex items-center rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-yu-blue-700 hover:text-yu-blue-700"
                          >
                        ...
                        {platformFeatures.privateBooking ? (
                          <Link
                            ...
                            className="inline-flex items-center rounded-xl bg-yu-blue-700 px-4 py-2 text-sm font-bold text-white transition hover:bg-yu-blue-600"
                          >
                            {t("courseDetails.instructor.bookSession", { defaultValue: isRtl ? "طلب جلسة فردية" : "Request a private session" })}
                          </Link>
                        ) : null}
                      </div>
                    ) : null}
              ...
              <div className="relative overflow-hidden bg-gradient-to-br from-yu-blue-950 to-yu-blue-700" style={{ paddingTop: "56.25%" }}>
                ...
                <button type="button" className="absolute inset-0 flex items-center justify-center">
                  <span className="flex h-14 w-14 items-center justify-center rounded-full bg-white/90 shadow-lg transition hover:scale-105">
                    <Play className="ms-1 h-6 w-6 fill-yu-blue-700 text-yu-blue-700" />
                  </span>
                </button>
              ...
              <div className="p-5">
                        ...
                        <label
                          ...
                          className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${
                            selectedTierId === tier.id
                              ? "border-[var(--yu-blue-700)] bg-orange-50/10 dark:bg-orange-500/5"
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              className="text-[var(--yu-blue-700)] focus:ring-[var(--yu-blue-700)] h-3.5 w-3.5"
                            />
                            <span className="text-xs font-bold text-slate-800 dark:text-white">
                              {isRtl ? (tier.nameAr || tier.name) : tier.name}
                            </span>
                          </div>
                          <span className="text-xs font-extrabold text-[var(--yu-blue-700)]">
                ...
                {showAuthHydrating ? (
                  ...
                  <div className="space-y-2">
                    <Link
                      to={loginHref}
                      className="flex w-full items-center justify-center gap-2 rounded-xl bg-yu-blue-700 py-3 text-sm font-bold text-white transition hover:bg-yu-blue-600"
                    >
                    ...
                    <Link
                      ...
                      className="flex w-full items-center justify-center rounded-xl border border-slate-200 py-3 text-sm font-semibold text-slate-800 transition hover:border-yu-blue-700 hover:text-yu-blue-700 dark:border-white/10 dark:text-slate-200"
                    >
                  ...
                  <Link
                    ...
                    className="flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--yu-blue-700)] py-3 text-sm font-bold text-white transition hover:bg-[var(--yu-blue-600)]"
                  >
                    <BookOpen className="h-4 w-4" />
                ...
                <ul className="space-y-2.5">
                  {INCLUSIONS.map(({ icon: Icon, key }) => (
                    <li key={key} className="flex items-start gap-2.5 text-sm text-slate-600">
                      <Icon className="mt-0.5 h-4 w-4 shrink-0 text-yu-blue-700" />
