      ...
      <main className="relative mx-auto -mt-10 max-w-[1320px] px-4 md:px-6 lg:px-8">
              ...
              {categoryTabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`shrink-0 whitespace-nowrap rounded-xl px-4 py-2.5 text-xs font-black transition ${
                      isActive ? "bg-[#071a38] text-white shadow-md" : "bg-slate-50 text-slate-500 hover:bg-blue-50 hover:text-blue-700"
                    }`}
                  >
                    {tab.label}
                  </button>
        ...
        <div className="mb-6 mt-10 flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-950">{isRtl ? "اختر الدورة المناسبة لمرحلتك" : "Choose the course for your stage"}</h2>
          </div>
        ...
        {isLoading ? (
          ...
          <>
            <div className={`grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 ${isFetching ? "opacity-70" : ""}`}>
              {sorted.map((course) => (
                <CourseCard
                  key={course.id}
                  course={course}
