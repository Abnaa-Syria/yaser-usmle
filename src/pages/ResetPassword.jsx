...
export default function ResetPassword() {
  ...
  const onSubmit = async ({ newPassword, confirmPassword }) => {
    setServerError("");
    try {
      await api.post(`${endpoints.auth.resetPassword}/${token}`, {
        newPassword,
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-pioneer-light-bg px-6 py-12 dark:bg-pioneer-dark-bg">
      <Link to="/" className="mb-8">
        <BrandLogo variant="primary" className="h-12 w-auto dark:hidden" />
        <BrandLogo variant="light" className="hidden h-12 w-auto dark:block" />
      </Link>

      <div className="w-full max-w-md rounded-2xl border border-slate-100 bg-white p-8 shadow-sm dark:border-white/10 dark:bg-slate-900">
        {done ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-5" noValidate>
              {serverError ? (
                <div className="rounded-lg bg-yu-blue-100 px-4 py-3 text-sm text-red-600">{serverError}</div>
              ) : null}
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-lg bg-yu-blue-700 py-3.5 text-sm font-semibold text-white disabled:opacity-60"
              >
