...
import { AlertCircle, ArrowLeft, ArrowRight, CheckCircle2, Clock3, Flag, Moon, Sun } from "lucide-react";
  ...
  return (
    <button
      ...
      className={`flex w-full items-center gap-4 rounded-xl border-2 px-5 py-4 text-start transition-all ${
        selected
          ? "border-yu-blue-700 bg-yu-blue-100 dark:border-[var(--yu-blue-500)] dark:bg-[var(--yu-blue-700)]/25"
          : "border-slate-200 bg-white hover:border-yu-blue-700/40 hover:bg-slate-50 dark:border-white/10 dark:bg-white/5 dark:hover:border-[var(--yu-blue-500)]/50 dark:hover:bg-white/10"
      }`}
    >
      <span
        className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 transition-colors ${
          selected ? "border-yu-blue-700 bg-yu-blue-700 dark:border-[var(--yu-blue-400)] dark:bg-[var(--yu-blue-500)]" : "border-slate-300 dark:border-white/30"
        }`}
      >
        {selected ? <span className="h-2 w-2 rounded-full bg-white" /> : null}
      </span>
      <span className={`text-sm font-medium leading-snug ${selected ? "text-yu-blue-950 dark:text-white" : "text-slate-700 dark:text-slate-200"}`}>{label}</span>
    </button>
  );
}
...
export default function TakeExam() {
  ...
  const TimerStrip = (
    <div className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/95 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-[#0B1628]/95">
      <div className="mx-auto max-w-7xl px-4 md:px-6 lg:px-8">
        <div className="flex h-14 items-center justify-between gap-4">
          <div className="min-w-0 flex-1">
            <p className="truncate text-base font-bold leading-tight text-slate-900 dark:text-white">{exam?.title}</p>
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <div
              className={`flex items-center gap-1.5 rounded-xl border px-3 py-2 text-sm font-bold tabular-nums transition ${
                isLow
                  ? "animate-pulse border-yu-blue-700 bg-yu-blue-100 text-yu-blue-700 dark:border-[var(--yu-blue-400)] dark:bg-[var(--yu-blue-700)]/30 dark:text-[var(--yu-blue-200)]"
  ...
  if (phase === "loading") {
    return (
      <div className="relative flex min-h-screen items-center justify-center bg-slate-50 text-slate-600 dark:bg-[#07111F] dark:text-slate-300">
        <div className="absolute end-4 top-4">{ThemeToggle}</div>
  ...
  if (phase === "error" || !exam) {
    return (
      <div className="mx-auto max-w-lg px-4 py-20 text-center dark:text-slate-200">
        <div className="mb-4 flex justify-end">{ThemeToggle}</div>
        <p className="text-red-600 dark:text-red-400">{getErrorMessage(error, t("takeExam.errors.load"))}</p>
        <button type="button" onClick={() => void refetch()} className="mt-4 text-yu-blue-700 hover:underline dark:text-[var(--yu-blue-300)]">
          {t("takeExam.retry", { defaultValue: "Retry" })}
        </button>
        <Link to={examsBase} className="mt-6 block text-sm text-slate-500 hover:text-yu-blue-700 dark:text-slate-400 dark:hover:text-[var(--yu-blue-300)]">
  ...
  if (phase === "expiredIncomplete") {
    return (
      <div className="mx-auto max-w-lg px-4 py-20 text-center">
        ...
        <h1 className="mt-4 text-xl font-bold text-slate-900 dark:text-white">{t("takeExam.expiredIncomplete.title")}</h1>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{t("takeExam.expiredIncomplete.body")}</p>
  ...
  if (phase === "intro") {
    ...
    if (startExam.isPending || (autostart && canStart)) {
      return (
        <div className="flex min-h-screen flex-col items-center justify-center gap-3 bg-slate-50 text-slate-600 dark:bg-[#07111F] dark:text-slate-300">
          <div className="absolute end-4 top-4">{ThemeToggle}</div>
      ...
      <div className="min-h-screen bg-gradient-to-b from-[var(--yu-blue-50)]/40 to-slate-50 py-12 md:py-16 dark:from-[#07111F] dark:to-[#0A1424]">
        <div className="mx-auto max-w-2xl space-y-4 px-4">
          ...
          <div className="overflow-hidden rounded-[1.75rem] border border-slate-200/80 bg-white shadow-[var(--shadow-md)] dark:border-white/10 dark:bg-[#0F1E38]">
            <div className="border-b border-slate-100 bg-gradient-to-r from-[var(--yu-blue-700)] to-[var(--yu-blue-500)] px-6 py-8 text-white md:px-8 dark:border-white/5">
            ...
            <div className="grid gap-4 px-6 py-6 sm:grid-cols-3 md:px-8">
              <div className="rounded-2xl bg-slate-50 px-4 py-4 text-center dark:bg-white/5">
                <p className="text-2xl font-black text-slate-900 dark:text-white">{exam.durationMinutes}</p>
                <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">{t("takeExam.meta.minutes")}</p>
              </div>
              <div className="rounded-2xl bg-slate-50 px-4 py-4 text-center dark:bg-white/5">
                <p className="text-2xl font-black text-slate-900 dark:text-white">{total}</p>
                <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">{t("takeExam.meta.questions")}</p>
              </div>
              <div className="rounded-2xl bg-slate-50 px-4 py-4 text-center dark:bg-white/5">
                <p className="text-2xl font-black text-slate-900 dark:text-white">{exam.passingScore}</p>
                <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">{t("takeExam.meta.passing")}</p>
              </div>
            </div>
            <div className="border-t border-slate-100 px-6 py-6 md:px-8 dark:border-white/10">
              {localErr ? <p className="mb-4 text-sm text-red-600 dark:text-red-400">{localErr}</p> : null}
              {!canStart ? <p className="mb-4 text-sm text-amber-700 dark:text-amber-300">{t("takeExam.notAvailable", { status: exam.status })}</p> : null}
              <button
                type="button"
              ...
              <Link to={`${examsBase}/${examId}`} className="mt-4 block text-center text-sm text-slate-500 hover:text-yu-blue-700 dark:text-slate-400 dark:hover:text-[var(--yu-blue-300)]">
  ...
  if (phase === "done" && result) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-[var(--yu-blue-50)]/30 to-slate-50 py-12 md:py-16 dark:from-[#07111F] dark:to-[#0A1424]">
        <div className="mx-auto max-w-2xl px-4 text-center">
          ...
          <div className="overflow-hidden rounded-[1.75rem] border border-slate-200/80 bg-white shadow-[var(--shadow-md)] dark:border-white/10 dark:bg-[#0F1E38]">
            ...
            <div className="grid grid-cols-3 gap-0 divide-x divide-slate-100 border-b border-slate-100 dark:divide-white/10 dark:border-white/10">
              <div className="py-6">
                <p className="text-2xl font-extrabold text-slate-900 dark:text-white">{scoreVal}/{maxPts}</p>
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{t("takeExam.result.score")}</p>
              </div>
              <div className="py-6">
                <p className="text-2xl font-extrabold text-slate-900 dark:text-white">{pct}%</p>
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{t("takeExam.result.percent")}</p>
              </div>
              <div className="py-6">
                <p className="text-2xl font-extrabold text-slate-900 dark:text-white">{exam.passingScore}</p>
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{t("takeExam.result.passing")}</p>
              </div>
            </div>
            <div className="space-y-3 px-6 py-8 md:px-8">
              {result.xp?.amount ? (
                <p className="inline-flex items-center justify-center gap-2 rounded-full border border-[var(--yu-blue-200)] bg-[var(--yu-blue-50)] px-4 py-2 text-xs font-bold text-[var(--yu-blue-800)] dark:border-[var(--yu-blue-700)] dark:bg-[var(--yu-blue-700)]/20 dark:text-[var(--yu-blue-200)]">
              ...
              <Link
                to={examsBase}
                className="block w-full rounded-xl border border-slate-200 py-3 text-sm font-semibold text-slate-600 transition hover:border-yu-blue-700 hover:text-yu-blue-700 dark:border-white/15 dark:text-slate-300 dark:hover:border-[var(--yu-blue-400)] dark:hover:text-white"
              >
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#07111F]">
        ...
        <div className="mx-auto max-w-3xl px-4 py-8 md:px-6">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">{t("takeExam.review.title", { defaultValue: "Review before submit" })}</h2>
          ...
          {localErr ? <p className="mt-4 text-sm text-red-600 dark:text-red-400">{localErr}</p> : null}
          ...
          <div className="mt-4 space-y-3">
                  ...
                  {unanswered.map((q) => {
                    ...
                    return (
                      <button
                        ...
                        className="rounded-lg bg-white px-3 py-1 text-xs font-bold text-amber-800 dark:bg-white/10 dark:text-amber-200"
                      >
                        Q{idx + 1}
                      </button>
                    );
            ...
            {flaggedList.length > 0 ? (
              <div className="rounded-xl border border-yu-blue-100 bg-yu-blue-50 p-4 text-sm text-yu-blue-900 dark:border-[var(--yu-blue-700)]/40 dark:bg-[var(--yu-blue-700)]/15 dark:text-[var(--yu-blue-200)]">
                  ...
                  {flaggedList.map((q) => {
                    ...
                    return (
                      <button
                        ...
                        className="rounded-lg bg-white px-3 py-1 text-xs font-bold text-yu-blue-700 dark:bg-white/10 dark:text-[var(--yu-blue-300)]"
                      >
                        Q{idx + 1}
                      </button>
                    );
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <button
              ...
              className="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 hover:border-yu-blue-700 hover:text-yu-blue-700 dark:border-white/15 dark:text-slate-300 dark:hover:border-[var(--yu-blue-400)] dark:hover:text-white"
            >
  ...
  if (!current) {
    return (
      <div className="relative p-8 text-center text-slate-600 dark:text-slate-300">
        <div className="absolute end-4 top-4">{ThemeToggle}</div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#07111F]">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 md:px-6">
        {localErr ? <p className="mb-4 text-center text-sm text-red-600 dark:text-red-400">{localErr}</p> : null}
        {Navigator}
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm dark:border-white/10 dark:bg-[#0F1E38]"
          >
            <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-6 py-3 md:px-8 dark:border-white/10">
              <h2 className="text-lg font-bold text-slate-900 md:text-xl dark:text-white">{t("takeExam.question.label", { n: currentIdx + 1 })}</h2>
              <button
                type="button"
                ...
                className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-bold transition ${
                  isFlagged
                    ...
                    : "border-slate-200 text-slate-500 hover:border-amber-300 hover:text-amber-700 dark:border-white/15 dark:text-slate-400 dark:hover:border-amber-400/50 dark:hover:text-amber-300"
                }`}
              >
            ...
            <div className="p-6 md:p-8">
              ...
              <p className="text-base font-semibold leading-relaxed text-slate-800 md:text-lg dark:text-slate-100">{current.questionText}</p>
              ...
              {(current.type === "MULTIPLE_CHOICE" || current.type === "TRUE_FALSE") && (
                <div className="mt-6 space-y-3">
                  {current.type === "TRUE_FALSE" ? (
                    ...
                    <p className="text-sm text-amber-700 dark:text-amber-300">{t("takeExam.noMcqOptions")}</p>
                  )}
                </div>
              )}
              ...
              {(current.type === "SHORT_ANSWER" || current.type === "ESSAY") && (
                <textarea
                  ...
                  className="mt-6 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none focus:border-yu-blue-700 focus:ring-2 focus:ring-yu-blue-100 dark:border-white/15 dark:bg-white/5 dark:text-white dark:placeholder:text-slate-500 dark:focus:border-[var(--yu-blue-500)] dark:focus:ring-[var(--yu-blue-700)]/30"
                  placeholder={t("takeExam.shortAnswerPlaceholder")}
                />
              )}
            </div>
            <div className="flex items-center justify-between gap-4 border-t border-slate-100 px-6 py-4 md:px-8 dark:border-white/10">
              <button
                type="button"
                ...
                className={`flex items-center gap-2 rounded-xl border-2 px-5 py-2.5 text-sm font-semibold ${
                  isFirst
                    ...
                    : "border-slate-200 text-slate-600 hover:border-yu-blue-700 hover:text-yu-blue-700 dark:border-white/15 dark:text-slate-300 dark:hover:border-[var(--yu-blue-400)] dark:hover:text-white"
                }`}
              >
