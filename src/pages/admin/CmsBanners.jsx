    ...
    <section className="space-y-6">
      ...
      {!isLoading && !sortedRows.length ? (
        <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-6 py-12 text-center dark:border-white/10 dark:bg-white/5">
          ...
          <button
            type="button"
            onClick={openCreate}
            className="inline-flex items-center gap-2 rounded-lg bg-[var(--yu-blue-700)] px-5 py-2.5 text-sm font-bold text-white"
          >
            <Plus className="h-4 w-4" />
            {t("adminPages.cmsBanners.add")}
          </button>
      ...
      >
        <form onSubmit={submitForm} className="space-y-4">
          ...
          <ImageField
            label={t("adminPages.cmsBanners.imageUrl")}
            required
            value={form.imageUrl}
          ...
          <button
            ...
            className="w-full rounded-lg bg-[var(--yu-blue-700)] py-3 text-sm font-bold text-white disabled:opacity-50"
          >
            {editingId
