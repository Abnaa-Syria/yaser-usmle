...
function InContextExamBuilder({ targetId, targetType, onClose, onCreated }) {
  ...
  const modal = (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm antialiased">
      <div
        role="dialog"
        aria-modal="true"
        className="flex w-full max-w-lg flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-white/8 dark:bg-[#1A1A22]"
      >
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-white/5">
          <div>
            ...
            <p className="text-[10px] uppercase tracking-widest text-slate-500">
              {t("adminPages.courseEditor.exam.settingsStepHint", {
                defaultValue: "Step 1: exam settings — then add questions",
              })}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-6">
          ...
          <label className="block space-y-1.5">
            ...
            <input
              ...
              placeholder={t("adminPages.courseEditor.exam.titlePlaceholder", {
                defaultValue: "e.g. Midterm assessment",
              })}
              className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm font-medium outline-none focus:border-[var(--yu-blue-700)] dark:border-white/10 dark:bg-[#0F0F13] dark:text-white"
            />
          </label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <label className="block space-y-1.5">
              ...
              <input
                ...
                className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm outline-none focus:border-[var(--yu-blue-700)] dark:border-white/10 dark:bg-[#0F0F13] dark:text-white"
              />
            </label>
            <label className="block space-y-1.5">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
          ...
          <div className="flex justify-end gap-3 border-t border-slate-100 pt-4 dark:border-white/5">
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl px-5 py-2.5 text-xs font-bold text-slate-400 hover:text-slate-600"
            >
            ...
            <button
              type="submit"
              disabled={isPending}
              className="inline-flex items-center gap-2 rounded-xl bg-[var(--yu-blue-700)] px-6 py-2.5 text-xs font-bold text-white shadow-lg shadow-[var(--yu-blue-700)]/20 hover:bg-[var(--yu-blue-600)] disabled:opacity-50"
            >
              {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
              {t("adminPages.courseEditor.exam.createAndEditQuestions", {
                defaultValue: "Create & edit questions",
              })}
            </button>
          </div>
        </form>
        ...
        <div className="space-y-3">
          {linkedExams.map((exam) => (
            <div key={exam.id} className="group relative overflow-hidden rounded-xl border border-purple-200 bg-white p-4 transition-all hover:border-purple-300 hover:shadow-md dark:border-purple-500/20 dark:bg-[#0F0F13]">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => handleUnlink(exam.id)} className="rounded-lg p-1 text-slate-400 hover:bg-[var(--yu-blue-700)]/10 hover:text-red-500">
          ...
          <div className="grid grid-cols-2 gap-2 pt-1">
            <button
              ...
              className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-[var(--yu-blue-700)] py-2 text-[11px] font-bold text-white hover:bg-[var(--yu-blue-600)]"
            >
  ...
  return (
    <div
      className={`group flex w-full items-center gap-2 rounded-lg transition-all ${
        isSelected
          ? "bg-[var(--yu-blue-700)]/10 text-[var(--yu-blue-700)] font-bold dark:bg-[var(--yu-blue-700)]/20 dark:text-red-300"
    ...
    >
      ...
      >
        ...
        {lesson.vdoCipherVideoId || lesson.videoUrl ? <Video className="h-3.5 w-3.5 shrink-0 text-emerald-500" /> : null}
      </button>
      <button
...
function UnitAccordion({
  ...
  return (
    <div className={`rounded-xl border transition-all ${isSelected ? 'border-[var(--yu-blue-700)]/30 bg-[var(--yu-blue-700)]/5 ring-1 ring-[var(--yu-blue-700)]/10' : 'border-slate-200 bg-white dark:border-white/8 dark:bg-[#1A1A22]'}`}>
      <div 
      ...
      >
        ...
        <Layers className={`h-4 w-4 shrink-0 ${isSelected ? 'text-[var(--yu-blue-700)]' : 'text-slate-400'}`} />
        <InlineEdit
          value={unit.title}
          onSave={(v) => onRenameUnit(unit.id, v)}
          placeholder={t("adminPages.courseEditor.empty.untitledUnit", { defaultValue: "Untitled unit" })}
          className={`min-w-0 flex-1 text-sm font-bold ${isSelected ? 'text-[var(--yu-blue-700)]' : 'text-slate-900 dark:text-white'}`}
        />
            ...
            sections.map((section) => (
              <div key={section.id} className="mb-2 rounded-lg border border-slate-100 bg-slate-50/50 p-2 dark:border-white/5 dark:bg-white/[0.02]">
                ...
                <button
                  ...
                  className="mt-1.5 inline-flex items-center gap-1 rounded-lg px-2 py-1 text-[10px] font-bold text-[var(--yu-blue-700)] hover:bg-[var(--yu-blue-700)]/10 disabled:opacity-50"
                >
...
function InlineEdit({ value, onSave, placeholder, className = "" }) {
  ...
  return (
    <input
      ...
      className={`rounded border border-[var(--yu-blue-700)]/40 bg-transparent px-2 py-0.5 outline-none ring-1 ring-[var(--yu-blue-700)]/20 ${className}`}
    />
  );
}
  ...
  useEffect(() => {
    if (node?.data) {
      setFormData({
        ...
        vdoCipherVideoId: node.data.vdoCipherVideoId || "",
    ...
    try {
      if (node.type === "course") {
        ...
        await updateLessonMutation.mutateAsync({ 
          ...
          body: { 
            title: formData.title, 
            vdoCipherVideoId: formData.vdoCipherVideoId.trim() || null,
    ...
    <div className="flex h-full flex-col antialiased">
        ...
        <div className="space-y-4">
          <label className="block space-y-1.5">
            ...
            <input
              ...
              className="h-11 w-full rounded-lg border border-slate-200 bg-white px-4 text-sm font-medium text-slate-900 outline-none transition-all focus:border-[var(--yu-blue-700)] focus:ring-1 focus:ring-[var(--yu-blue-700)]/20 dark:border-white/10 dark:bg-[#0F0F13] dark:text-white"
            />
          </label>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded bg-[var(--yu-blue-700)]/10 border border-[var(--yu-blue-700)]/30 px-2.5 py-1 text-xs font-bold text-[var(--yu-blue-700)] hover:bg-[var(--yu-blue-700)]/20 transition"
                    >
                      + ٦ أشهر
                    </button>
                    <button
                ...
                <div className="space-y-3">
                  {(formData.pricingTiers || []).map((tier, idx) => (
                    <div key={idx} className="grid grid-cols-1 md:grid-cols-5 gap-3 p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/40 relative">
                      ...
                      <label className="block space-y-1 md:col-span-1">
                        <span className="text-[9px] font-bold text-slate-400">السعر (USD)</span>
                        <input
                          type="number"
                          min={0}
                          value={tier.price}
              ...
              <div className="rounded-xl border border-slate-200 p-3 dark:border-white/10">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className="rounded-lg bg-[var(--yu-blue-700)] px-3 py-2 text-xs font-bold text-white disabled:opacity-50"
                  >
          ...
          {node.type === "lesson" && (
            <>
              <label className="block space-y-1.5">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
                  {t("adminPages.courseEditor.fields.vdoCipherVideoId", { defaultValue: "VdoCipher Video ID" })}
                </span>
                <div className="relative">
                  <Video className="pointer-events-none absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                  <input
                    value={formData.vdoCipherVideoId}
                    onChange={(e) => setFormData({ ...formData, vdoCipherVideoId: e.target.value })}
                ...
                <VdoCipherVideoPicker
                  value={formData.vdoCipherVideoId}
                  onSelect={(video) => {
                    setFormData((prev) => ({
                      ...prev,
                      vdoCipherVideoId: video.id,
                    }));
                  }}
                />
                <p className="text-[10px] text-slate-400">
                  {t("adminPages.courseEditor.fields.vdoCipherHint", {
                    defaultValue: "Paste the video ID from your VdoCipher dashboard, or pick from your library. Playback uses secure OTP.",
                  })}
                </p>
              </label>
              <label className="block space-y-1.5">
                <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
                  {t("adminPages.courseEditor.fields.videoUrlLegacy", { defaultValue: "Legacy Video URL (optional)" })}
                </span>
                <div className="relative">
                  <Video className="pointer-events-none absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                  <input
                    ...
                    placeholder="https://… (only if not using VdoCipher)"
                    className="h-11 w-full rounded-lg border border-slate-200 bg-white ps-10 pe-4 text-sm dark:border-white/10 dark:bg-[#0F0F13] dark:text-white"
                  />
                </div>
              </label>
      ...
      <div className="border-t border-slate-200 px-5 py-4 dark:border-white/8">
        <button
          ...
          className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[var(--yu-blue-700)] px-4 py-2.5 text-sm font-bold text-white transition-all hover:bg-[var(--yu-blue-600)] disabled:opacity-50"
        >
          {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
          {saved
...
export default function CourseEditor() {
  ...
  if (isLoading) return <div className="flex min-h-[60vh] items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-[var(--yu-blue-700)]" /></div>;
  if (isError || !course) return (
    <div className="mx-auto max-w-lg space-y-4 py-20 text-center">
      ...
      <Link to="/admin/courses" className="text-sm text-[var(--yu-blue-700)] hover:underline">
    ...
    <section className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          { label: t("adminPages.courseEditor.stats.videos", { defaultValue: "Videos" }), value: units.reduce((s, u) => s + ((u.sections || []).reduce((secSum, sec) => secSum + ((sec.lessons || []).filter((l) => l.vdoCipherVideoId || l.videoUrl).length), 0)), 0), color: "text-purple-500" },
      ...
      <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={handleAddUnit} className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-[var(--yu-blue-700)] px-3 py-1.5 text-xs font-bold text-white hover:bg-[var(--yu-blue-600)]">
          ...
          <button 
            ...
            className={`flex w-full items-center gap-3 rounded-xl border p-4 text-start transition-all ${selectedNode?.type === 'course' ? 'border-[var(--yu-blue-700)] bg-[var(--yu-blue-700)]/5 ring-1 ring-[var(--yu-blue-700)]/10' : 'border-slate-200 bg-white hover:bg-slate-50 dark:border-white/8 dark:bg-[#1A1A22]'}`}
          >
            <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${selectedNode?.type === 'course' ? 'bg-[var(--yu-blue-700)] text-white' : 'bg-slate-100 text-slate-500 dark:bg-white/10'}`}><Info className="h-5 w-5" /></div>
            <div className="min-w-0">
              <p className={`text-sm font-bold ${selectedNode?.type === 'course' ? 'text-[var(--yu-blue-700)]' : 'text-slate-900 dark:text-white'}`}>
