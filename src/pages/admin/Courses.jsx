...
function Courses() {
  ...
  const total = data?.meta?.total ?? data?.courses?.length ?? 0;
  const totalPages = Math.max(1, Number(data?.meta?.totalPages) || Math.ceil(total / query.limit) || 1);
  const page = Number(data?.meta?.page) || query.page;
        ...
        actions={
          <div className="relative">
            ...
            <input
              ...
              className="h-10 w-64 rounded-xl border border-slate-200/80 bg-white ps-9 pe-4 text-sm font-medium text-slate-900 outline-none transition-all placeholder:text-slate-400 focus:border-[var(--yu-blue-700)] focus:ring-1 focus:ring-[var(--yu-blue-700)]/20 shadow-sm dark:border-white/10 dark:bg-[#1A1A22] dark:text-white"
            />
          </div>
        }
      />
        ...
        columns={[
            ...
            render: (value, row) => (
              <div className="group/item relative flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <Link to={`/admin/courses/${row.id}/edit`} className="font-bold text-slate-900 dark:text-white line-clamp-1 hover:text-[var(--yu-blue-700)] transition-colors">{value}</Link>
          ...
          ...(multiInstructor
            ? [
                {
                  key: "instructor",
                  title: t("dashboard.admin.courses.instructor"),
                  render: (_, row) => row?.instructor?.fullName || "-",
                },
              ]
            : []),
          {
                  ...
                  render: (_, row) => (
                    <div className="relative">
                      <select
                        ...
                        className="h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-xs text-slate-700 outline-none focus:border-[var(--yu-blue-700)] disabled:opacity-60 dark:border-white/10 dark:bg-[#0F0F13] dark:text-white"
                      >
                      ...
                      {assignMutation.isPending && assignMutation.variables?.id === row.id && (
                        <Loader2 className="absolute end-8 top-1/2 h-3 w-3 -translate-y-1/2 animate-spin text-[var(--yu-blue-700)]" />
                      )}
                    </div>
                  ),
                },
            ...
            render: (_, row) => (
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="inline-flex rounded-md p-2 text-red-500 hover:bg-[var(--yu-blue-700)]/10 dark:hover:bg-[var(--yu-blue-700)]/10"
                >
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-slate-600 dark:text-slate-400">
              <span>
                ...
                <span className="ms-2 text-xs text-slate-400">
                  ({total.toLocaleString()} {isRtl ? "دورة" : "courses"})
                </span>
              </span>
              <div className="flex items-center gap-2">
                ...
                >
                  {t("adminPages.pagination.prev", { defaultValue: isRtl ? "السابق" : "Previous" })}
                </button>
                <button
                  type="button"
                  ...
                  className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold disabled:opacity-40 dark:border-white/10 dark:text-slate-300 dark:hover:bg-white/5"
                >
                  {t("adminPages.pagination.next", { defaultValue: isRtl ? "التالي" : "Next" })}
                </button>
              </div>
            </div>
          ) : null
