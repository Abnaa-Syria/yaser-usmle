...
function Instructors() {
  ...
  const canAddInstructor = platformFeatures.multiInstructor;
  ...
  const columns = useMemo(
    () => [
      { key: "fullName", title: t("dashboard.admin.users.name"), render: (value, row) => (
        <Link to={`/admin/instructors/${row.id}`} className="font-semibold text-[var(--yu-blue-700)] hover:underline">
          {value || "—"}
        </Link>
      ) },
        ...
        render: (_, row) => (
          <div className="flex items-center gap-2">
            <Link
              ...
              className="rounded-lg border border-white/5 bg-white/5 p-2 text-slate-400 transition-all hover:bg-[var(--yu-blue-700)] hover:text-white"
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="h-10 w-64 rounded-lg border border-slate-200 bg-white ps-9 pe-4 text-sm font-medium text-slate-900 outline-none transition-all placeholder:text-slate-400 focus:border-[var(--yu-blue-700)] focus:ring-1 focus:ring-[#3B82F6]/30 dark:border-slate-800 dark:bg-[#1E293B] dark:text-white dark:placeholder:text-slate-600 dark:focus:border-[var(--yu-blue-700)] dark:focus:ring-[#3B82F6]/30"
              />
            </div>
            {canAddInstructor ? (
            <button onClick={openCreate} className="inline-flex h-10 items-center gap-2 rounded-lg bg-[var(--yu-blue-700)] px-5 text-sm font-bold text-white shadow-xl shadow-[var(--yu-blue-700)]/20 transition-all hover:bg-[var(--yu-blue-600)] active:scale-95">
          ...
          <form onSubmit={onSubmit} className="w-full max-w-2xl rounded-xl border border-slate-200 bg-white p-8 shadow-2xl dark:border-slate-800 dark:bg-[#1E293B]">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {["fullName", "email", "phone", "experience"].map((field) => (
                <div key={field} className="space-y-1.5">
                  ...
                  <input 
                    {...form.register(field)} 
                    className="h-11 w-full rounded-lg border border-slate-200 bg-white px-4 text-sm font-medium text-slate-900 outline-none transition-all focus:border-[var(--yu-blue-700)] focus:ring-1 focus:ring-[#3B82F6]/30 dark:border-slate-800 dark:bg-[#1E293B] dark:text-white dark:focus:border-[var(--yu-blue-700)] dark:focus:ring-[#3B82F6]/30"
                  />
                </div>
              ))}
              ...
              <div className="space-y-1.5 sm:col-span-2">
                ...
                <textarea 
                  {...form.register("bio")} 
                  className="min-h-24 w-full rounded-lg border border-slate-200 bg-white p-4 text-sm font-medium text-slate-900 outline-none transition-all focus:border-[var(--yu-blue-700)] focus:ring-1 focus:ring-[#3B82F6]/30 dark:border-slate-800 dark:bg-[#1E293B] dark:text-white dark:focus:border-[var(--yu-blue-700)] dark:focus:ring-[#3B82F6]/30"
                />
              </div>
            </div>
              ...
              <div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/30 dark:bg-amber-500/10">
                ...
                <div className="flex flex-col gap-2 sm:flex-row">
                  <input
                    ...
                    className="h-11 flex-1 rounded-lg border border-slate-200 bg-white px-4 text-sm font-medium text-slate-900 outline-none transition-all focus:border-[var(--yu-blue-700)] focus:ring-1 focus:ring-[#3B82F6]/30 dark:border-slate-800 dark:bg-[#1E293B] dark:text-white"
                  />
                  <button
                    type="button"
            ...
            <div className="mt-8 flex justify-end gap-3">
              ...
              <button 
                ...
                className="inline-flex items-center gap-2 rounded-lg bg-[var(--yu-blue-700)] px-6 py-2.5 text-sm font-bold text-white shadow-xl shadow-[var(--yu-blue-700)]/20 transition-all hover:bg-[var(--yu-blue-600)] active:scale-95"
              >
