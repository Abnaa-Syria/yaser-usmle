...
function InstructorsList() {
  ...
  return (
    <section className="space-y-6">
      <PageHeader
        title={t("adminPages.instructorsList.title")}
        subtitle={t("adminPages.instructorsList.subtitle")}
        action={
          platformFeatures.multiInstructor ? (
            <Link to="/admin/instructors" className="inline-flex items-center rounded-lg bg-[var(--yu-blue-700)] px-4 py-2 text-sm font-bold text-white">
              {t("adminPages.instructorsList.addInstructor")}
            </Link>
          ) : null
        }
      />
      ...
      {isError ? (
        <div className="rounded-xl border border-red-200 bg-[var(--yu-blue-700)]/10 p-4 text-sm text-red-700 dark:border-red-500/30 dark:bg-[var(--yu-blue-700)]/10 dark:text-red-300">
          ...
          <button onClick={() => refetch()} className="ms-3 rounded bg-[var(--yu-blue-700)] px-2 py-1 text-xs font-bold text-white">Retry</button>
        </div>
      ) : null}
            ...
            render: (_, r) => {
              ...
              return (
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[var(--yu-blue-700)]/90 to-amber-600/90 text-xs font-bold text-white shadow-sm">
                    {initials || "?"}
                  </div>
          ...
          { key: "actions", title: t("adminPages.common.actions"), render: (_, r) => (
            <div className="flex gap-2">
              <Link
                ...
                className="inline-flex items-center gap-1 rounded-md px-2 py-1.5 text-xs font-bold text-[var(--yu-blue-700)] hover:bg-[var(--yu-blue-700)]/10"
              >
