...
const BAR_COLORS = ["var(--yu-blue-700)", "#3B82F6", "#10B981", "#F59E0B", "#0F2448"];

function StatSkeleton() {
  return (
    <div className="h-[140px] animate-pulse rounded-[1.35rem] border border-slate-200/80 bg-slate-100/80 dark:border-white/8 dark:bg-white/5" />
  );
}
...
function ChartSkeleton({ className = "" }) {
  return (
    <div
      className={`animate-pulse rounded-[1.35rem] border border-slate-200/80 bg-slate-100/80 dark:border-white/8 dark:bg-white/5 ${className}`}
    />
  );
}
...
export default function Overview() {
  ...
  const securitySignals = useMemo(() => {
    ...
    return {
      concurrentSessions: openTickets,
      forceLogouts: statsData?.forceLogouts ?? 0,
    };
  }, [statsData?.openTickets, statsData?.forceLogouts]);

  const baseRevenue = summary?.totalRevenue || 0;
  const streamData = baseRevenue > 0
    ? [
        { name: t("overview.recordedCourses"), value: Math.round(baseRevenue * 0.45), color: "var(--yu-blue-700)" },
  ...
  const formattedTotalSales = `$${totalSales.toLocaleString(undefined, { maximumFractionDigits: 0 })}`;

  return (
    <section className="space-y-8 pb-10">
      <AdminPageHero
        eyebrow={t("header.dashboardMenu.adminPanel")}
        title={t("dashboard.admin.pages.overview.title")}
        subtitle={t("dashboard.admin.pages.overview.subtitle")}
      />

      {isError ? (
        <div className="rounded-[1.25rem] border border-red-200 bg-red-50/80 px-4 py-3 text-sm text-red-800 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-200">
          {error?.message || t("common.error", { defaultValue: "Something went wrong." })}
        </div>
      ) : null}

      {!canReadDashboard ? (
        <div className="flex flex-col items-center justify-center rounded-[1.5rem] border border-slate-200 bg-white/90 p-12 text-center shadow-[var(--shadow-sm)] dark:border-white/8 dark:bg-[#0F1E38]/85">
          <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[var(--yu-blue-700)]/10 text-[var(--yu-blue-700)]">
        ...
        <>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {isLoading || isLoadingStats ? (
              ...
              <>
                {/* Earnings Card */}
                <article className="group relative flex flex-col justify-between rounded-[1.35rem] border border-slate-200/90 bg-white/90 p-5 shadow-[var(--shadow-sm)] backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:shadow-[var(--shadow-md)] dark:border-white/8 dark:bg-[#0F1E38]/80 dark:hover:border-emerald-500/30">
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-[#1E293B]/60 dark:backdrop-blur-lg dark:hover:border-[var(--yu-blue-700)]/30 dark:hover:shadow-[0_0_20px_rgba(238,124,17,0.06)] transition-all duration-300 lg:col-span-2">
                  ...
                  {["24h", "7d", "30d", "1y"].map((filter) => (
                    <button
                      ...
                      className={`rounded px-2.5 py-1 text-[10px] font-bold uppercase transition duration-150 ${
                        timeFilter === filter
                          ? "bg-white text-[#1E293B] shadow-sm dark:bg-[var(--yu-blue-700)] dark:text-white dark:shadow-[0_0_10px_rgba(238,124,17,0.3)]"
                    ...
                    <AreaChart data={currentChartData}>
                      <defs>
                        <linearGradient id="adminRevenueGrad" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="var(--yu-blue-700)" stopOpacity={isDark ? 0.35 : 0.4} />
                      ...
                      <Tooltip
                        content={({ active, payload, label }) => {
                          ...
                          const formattedVal = `$${value.toLocaleString(undefined, { maximumFractionDigits: 0 })}`;
                          return (
                            <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs shadow-md dark:border-white/10 dark:bg-slate-950/90 dark:backdrop-blur-md z-50">
            ...
            <div className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-[#1E293B]/60 dark:backdrop-blur-lg dark:hover:border-[#3B82F6]/30 dark:hover:shadow-[0_0_20px_rgba(59,130,246,0.06)] transition-all duration-300 lg:col-span-1">
              <div className="mb-4">
                <h3 className="text-lg font-bold tracking-tight text-slate-900 dark:text-slate-100">
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            {/* Quick Actions Panel */}
            <div className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-[#1E293B]/60 dark:backdrop-blur-lg lg:col-span-1">
              <div className="mb-4">
                <h3 className="text-lg font-bold tracking-tight text-slate-900 dark:text-slate-100">
              ...
              <div className="grid grid-cols-1 gap-3">
                <Link
                  ...
                  className="flex items-center gap-3 rounded-lg border border-slate-200/60 bg-slate-50/55 p-3 text-sm font-semibold text-slate-800 transition duration-200 hover:border-[var(--yu-blue-700)]/40 hover:bg-[var(--yu-blue-700)]/5 dark:border-white/5 dark:bg-white/[0.01] dark:text-slate-200 dark:hover:border-[var(--yu-blue-700)]/40 dark:hover:bg-[var(--yu-blue-700)]/5 group"
                >
                  <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--yu-blue-700)]/10 text-[var(--yu-blue-700)] transition group-hover:scale-105">
            ...
            <div className="rounded-xl border border-slate-200/80 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-[#1E293B]/60 dark:backdrop-blur-lg lg:col-span-2">
              <div className="mb-6">
                <h3 className="text-lg font-bold tracking-tight text-slate-900 dark:text-slate-100">
                    ...
                    <BarChart data={enrollmentChartData} layout="vertical" margin={{ left: 8, right: 16 }}>
                      ...
                      <Tooltip
                        content={({ active, payload }) => {
                          ...
                          return (
                            <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs shadow-md dark:border-white/10 dark:bg-slate-950/90 dark:backdrop-blur-md z-50">
                              <p className="font-semibold text-slate-900 dark:text-slate-100">{row?.fullTitle}</p>
          ...
          <div className="grid gap-6 lg:grid-cols-5">
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800 dark:bg-[#1E293B]/60 dark:backdrop-blur-lg lg:col-span-3">
              <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/30 px-6 py-5 dark:border-white/5 dark:bg-white/[0.02]">
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-yu-blue-700/10 text-yu-blue-700">
                    <Activity className="h-5 w-5" />
                  </div>
                  <div>
                    <h3 className="text-lg font-bold tracking-tight text-slate-900 dark:text-slate-100">{t("overview.activityFeed")}</h3>
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                            <span className="text-yu-blue-700">{item.studentName}</span>
                            <span className="mx-1 text-slate-400">·</span>
                            {item.label}
                          </p>
                          {item.detail ? <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{item.detail}</p> : null}
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800 dark:bg-[#1E293B]/60 dark:backdrop-blur-lg lg:col-span-2 flex flex-col">
              <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/30 px-6 py-5 dark:border-white/5 dark:bg-white/[0.02]">
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-yu-blue-700/10 text-yu-blue-700">
                    <Terminal className="h-5 w-5" />
