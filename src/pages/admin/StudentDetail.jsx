...
import { useAdminEnrollments, useRevokeAdminEnrollment, useUpdateAdminEnrollmentExpiry } from "../../features/admin/enrollments/hooks";
...
function StudentDetail() {
  ...
  return (
    <section className="space-y-6 antialiased">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/8 dark:bg-[#1A1A22]">
              ...
              <p className="mt-2 text-xl font-black text-slate-900 dark:text-white">
                {sidebarStats.totalSpent.toLocaleString()} <span className="text-xs font-medium text-slate-500">USD</span>
              </p>
            </div>
          </div>
              ...
              <div className="space-y-2 rounded-2xl border border-[var(--yu-blue-700)]/10 bg-slate-50/50 p-5 dark:border-white/5 dark:bg-white/5">
                ...
                <PermissionGate permission="user:manage">
                  <button
                    ...
                    className="flex w-full items-center gap-2.5 rounded-xl border border-slate-200/80 bg-white px-4 py-3 text-sm font-bold text-slate-800 transition hover:border-[var(--yu-blue-700)]/40 dark:border-white/10 dark:bg-[#0F0F13] dark:text-slate-200"
                  >
        ...
        <div className="space-y-6">
          {perfQuery.isLoading ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-6 text-center text-slate-500 dark:border-white/8 dark:bg-[#1A1A22]">
              <div className="h-6 w-6 animate-spin rounded-full border-2 border-[var(--yu-blue-700)] border-t-transparent mx-auto"></div>
            ...
            <div className="rounded-2xl border border-red-200 bg-[var(--yu-blue-700)]/10 p-5 text-sm text-red-700 dark:border-red-500/30">
              ...
              <button onClick={() => perfQuery.refetch()} className="ms-3 rounded-lg bg-[var(--yu-blue-700)] px-3 py-1 text-xs font-bold text-white">
            ...
            <StudentPerformanceUI
              data={{
                ...
                recentGrades: (perfQuery.data.recentGrades || []).filter((g) => g.kind === "exam"),
              }}
            />
          ) : null}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-[#1A1A22]">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">{t("adminPages.studentDetail.resetPassword")}</h3>
            ...
            <input
              ...
              placeholder={t("adminPages.students.passwordMin", { defaultValue: "Min 8 characters" })}
            />
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="rounded-xl bg-[var(--yu-blue-700)] px-4 py-2 text-sm font-bold text-white disabled:opacity-60"
              >
                {t("adminPages.studentDetail.savePassword", { defaultValue: "Update password" })}
              </button>
            </div>
          </div>
        </div>
      ...
      {ticketModalOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <form onSubmit={handleCreateTicket} className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-[#1A1A22]">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">{t("adminPages.studentDetail.sendMessage")}</h3>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button type="submit" disabled={createTicketMutation.isPending} className="inline-flex items-center gap-2 rounded-xl bg-[var(--yu-blue-700)] px-4 py-2 text-sm font-bold text-white disabled:opacity-60">
      ...
      {deleteStep === 1 ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl border border-rose-200 bg-white p-6 shadow-2xl dark:border-rose-500/30 dark:bg-[#1A1A22]">
            <h3 className="text-lg font-bold text-rose-700 dark:text-rose-300">{t("adminPages.studentDetail.deleteConfirmTitle", { defaultValue: "Confirm deletion" })}</h3>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
              {t("adminPages.studentDetail.deleteTypeEmail", { defaultValue: "Type the student email exactly to confirm deletion." })}
            </p>
            <input
              value={deleteConfirmText}
              onChange={(e) => setDeleteConfirmText(e.target.value)}
              className="mt-4 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm dark:border-white/10 dark:bg-[#0F0F13] dark:text-white"
            />
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setDeleteStep(0)} className="rounded-xl border px-4 py-2 text-sm font-semibold dark:border-white/10 dark:text-slate-200">
                {t("dashboard.common.cancel")}
              </button>
              <button type="button" onClick={() => void handleDeleteConfirm()} className="rounded-xl bg-rose-600 px-4 py-2 text-sm font-bold text-white">
                {t("adminPages.studentDetail.delete", { defaultValue: "Delete" })}
              </button>
            </div>
          </div>
        </div>
