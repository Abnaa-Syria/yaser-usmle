  ...
  if (isLoading) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center text-slate-500 dark:border-slate-800 dark:bg-[#1E293B] font-bold flex items-center justify-center gap-2">
        <Clock className="h-5 w-5 animate-spin text-[var(--yu-blue-700)]" />
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-[#1E293B] shadow-md">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-bold ${
                ...
                ticket.priority === "HIGH" ? "bg-orange-500/10 text-[var(--yu-blue-700)] border border-orange-500/20" :
                "bg-slate-500/10 text-slate-500 border border-slate-500/20"
              }`}>
                {t(`adminPages.tickets.priorities.${ticket.priority || "MEDIUM"}`, {
          ...
          <div className="text-xs text-slate-450 dark:text-slate-500 flex flex-col items-end">
            ...
            <span className="mt-0.5">{t("adminPages.tickets.category")}: {ticket.category || t("adminPages.tickets.general")}</span>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col h-[650px] bg-white dark:bg-[#1E293B] rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xl overflow-hidden">
            ...
            <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 flex items-center justify-between">
              <h3 className="text-xs font-bold text-slate-500 dark:text-slate-450 uppercase tracking-widest flex items-center gap-2">
                ...
                {t("adminPages.tickets.conversationFeed")}
              </h3>
              <span className="text-[10px] text-slate-400 dark:text-slate-550">
                {t("adminPages.tickets.messagesCount", { count: messagesList.length })}
              </span>
            </div>
                  ...
                  >
                    ...
                    <div className={`flex items-center gap-2 mb-1.5 text-[11px] text-slate-450 dark:text-slate-500 ${
                      ...
                      <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[9px] font-black ${
                        isAdmin 
                          ? "bg-orange-500/15 text-[var(--yu-blue-700)]" 
                          : "bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400"
                      }`}>
                        {isAdmin ? "AD" : getInitials(ticket.creator?.fullName)}
                      </div>
                      <span className="font-bold text-slate-650 dark:text-slate-350">
                        {isAdmin ? (ticket?.assignedTo?.fullName || t("adminPages.tickets.adminSupport")) : (ticket?.creator?.fullName || t("adminPages.tickets.requester"))}
                      </span>
                      <span>•</span>
                    ...
                    <div className={`p-4 text-sm leading-relaxed shadow-xs ${
                      isAdmin 
                        ? "bg-[#0F172A] border-l-4 border-l-[var(--yu-blue-700)] border border-slate-200/50 dark:border-slate-800 text-white rounded-2xl rounded-tr-none"
            ...
            <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-[#1E293B] flex items-end gap-3">
              <textarea
                ...
                rows={2}
                placeholder={t("adminPages.tickets.replyPlaceholder")}
                className="min-h-11 flex-1 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-xl px-4 py-2.5 text-sm text-slate-800 dark:text-white outline-none focus:border-[var(--yu-blue-700)] focus:ring-1 focus:ring-[var(--yu-blue-700)] transition-all resize-none"
              />
              <button
                type="button"
                disabled={replyMutation.isPending || !reply.trim()}
                onClick={handleSendReply}
                className="h-11 w-11 flex items-center justify-center rounded-xl bg-[var(--yu-blue-700)] text-white hover:bg-[var(--yu-blue-600)] active:bg-[var(--yu-blue-700)] disabled:opacity-50 transition-all duration-200 transform hover:scale-105 active:scale-95 shadow-lg shadow-orange-500/20 shrink-0"
              >
        ...
        <aside className="lg:col-span-4 space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-[#1E293B] shadow-md space-y-4">
            <h3 className="text-xs font-bold text-slate-500 dark:text-slate-450 uppercase tracking-widest pb-3 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2">
              <UserIcon className="h-4 w-4 text-[var(--yu-blue-700)]" />
              {t("adminPages.tickets.requesterProfile")}
            </h3>
            
            <div className="space-y-3.5 text-sm">
              <div className="flex flex-col">
                <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider">
                  {t("adminPages.tickets.fullName")}
                </span>
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider">
                  {t("adminPages.tickets.emailAddress")}
                </span>
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider">
                  {t("adminPages.tickets.activeRole")}
                </span>
                <span className="inline-flex self-start items-center gap-1 mt-1.5 rounded-lg bg-orange-500/10 px-2.5 py-0.5 text-xs font-bold text-[var(--yu-blue-700)]">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider">
                  {t("adminPages.tickets.accountStatus")}
                </span>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-[#1E293B] shadow-md space-y-4">
            <h3 className="text-xs font-bold text-slate-500 dark:text-slate-450 uppercase tracking-widest pb-3 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2">
              ...
              {t("adminPages.tickets.ticketMetadata")}
            </h3>
            ...
            <div className="space-y-2 text-sm text-slate-600 dark:text-slate-350">
              <div className="flex justify-between py-1 border-b border-slate-100/50 dark:border-slate-800/40">
                <span>{t("adminPages.tickets.assignedAdmin")}</span>
                <span className="font-bold text-slate-800 dark:text-white">
                  {ticket?.assignedTo?.email || t("adminPages.tickets.unassigned")}
                </span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-100/50 dark:border-slate-800/40">
                <span>{t("adminPages.tickets.createdDate")}</span>
                <span>{formatDate(ticket.createdAt, isRtl)}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-100/50 dark:border-slate-800/40">
                <span>{t("adminPages.tickets.lastUpdate")}</span>
            ...
            <div className="mt-4 pt-2">
              <label className="text-[10px] font-bold text-slate-450 dark:text-slate-500 uppercase tracking-widest block mb-2">
                {t("adminPages.tickets.changeStatus")}
              </label>
              <select
                value={status}
                onChange={(e) => {
                  if (window.confirm(t("adminPages.tickets.confirmStatusChange"))) {
                    const next = e.target.value;
                    setStatus(next);
                    processMutation.mutate({ id, body: { status: next, response: notes || undefined } });
                  }
                }}
                className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none dark:border-white/10 dark:bg-slate-900 dark:text-slate-300 focus:border-[var(--yu-blue-700)] focus:ring-1 focus:ring-[var(--yu-blue-700)] transition-all"
              >
          ...
          <div className="rounded-2xl border border-amber-500/25 bg-amber-500/5 p-5 dark:border-amber-500/15 space-y-3">
            ...
            <textarea
              ...
              rows={4}
              placeholder={t("adminPages.tickets.internalNotesPlaceholder")}
              className="w-full rounded-xl border border-amber-500/10 bg-white p-3 text-sm text-slate-700 outline-none focus:border-[var(--yu-blue-700)] dark:border-white/5 dark:bg-slate-900 dark:text-slate-200 transition-all resize-none"
            />
          </div>
