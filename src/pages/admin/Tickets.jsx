...
const priorityClasses = {
  URGENT: "border border-red-500/20 bg-[var(--yu-blue-700)]/15 text-red-400",
  HIGH: "border border-red-500/20 bg-[var(--yu-blue-700)]/15 text-red-400",
    ...
    <PermissionGate 
      ...
      fallback={
        <p className="text-sm text-slate-500">
          {t("adminPages.tickets.noAccess", { defaultValue: "You do not have access to support management." })}
        </p>
      }
    >
      <section className="space-y-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
              {t("adminPages.tickets.title", { defaultValue: "Support Tickets" })}
            </h1>
            <p className="mt-1 text-sm text-slate-500">
              {t("adminPages.tickets.subtitle", { defaultValue: "Manage student and instructor support requests" })}
            </p>
          </div>
          <button className="inline-flex items-center gap-2 rounded-lg bg-[var(--yu-blue-700)] px-4 py-2 text-sm font-bold text-white transition hover:bg-[var(--yu-blue-600)]">
            <Plus className="h-4 w-4" />
            {t("adminPages.tickets.newBtn", { defaultValue: "New Ticket" })}
          </button>
        </div>

        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm dark:border-white/8 dark:bg-[#1A1A22]">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-slate-500">
                  {t("adminPages.tickets.statsOpen", { defaultValue: "Open Tickets" })}
                </p>
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm dark:border-white/8 dark:bg-[#1A1A22]">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-slate-500">
                  {t("adminPages.tickets.statsInProgress", { defaultValue: "In Progress" })}
                </p>
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm dark:border-white/8 dark:bg-[#1A1A22]">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-slate-500">
                  {t("adminPages.tickets.statsResolved", { defaultValue: "Resolved Today" })}
                </p>
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm dark:border-white/8 dark:bg-[#1A1A22]">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-slate-500">
                  {t("adminPages.tickets.statsTotal", { defaultValue: "Total This Month" })}
                </p>
          ...
          <div className="grid gap-3 lg:grid-cols-12">
            <div className="relative lg:col-span-4">
              ...
              <input
                ...
                placeholder={t("adminPages.tickets.searchPlaceholder", { defaultValue: "Search tickets..." })}
                className="h-10 w-full rounded-lg border border-slate-200 bg-white ps-9 pe-3 text-sm text-slate-900 outline-none focus:border-[var(--yu-blue-700)] dark:border-white/10 dark:bg-[#0F0F13] dark:text-white"
              />
            </div>
            <select 
              value={status} 
              onChange={(e) => setStatus(e.target.value)} 
              className="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none dark:border-white/10 dark:bg-[#0F0F13] dark:text-slate-300 lg:col-span-2"
            >
            ...
            >
              {PRIORITY_OPTIONS.map((p) => (
                <option key={p} value={p}>
                  {t(`adminPages.tickets.priorities.${p}`, { defaultValue: p })}
                </option>
              ))}
            </select>
        ...
        {isError ? (
          <div className="rounded-xl border border-red-200 bg-[var(--yu-blue-700)]/10 p-4 text-sm text-red-700 dark:border-red-500/30 dark:bg-[var(--yu-blue-700)]/10 dark:text-red-300">
            {getErrorMessage(error, t("adminPages.tickets.loadError", { defaultValue: "Failed to load tickets." }))}
            <button onClick={() => refetch()} className="ms-3 rounded bg-[var(--yu-blue-700)] px-2 py-1 text-xs font-bold text-white">
        ...
        {!isLoading && !isError && tickets.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white py-16 text-center dark:border-white/8 dark:bg-[#1A1A22]">
            ...
            <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">
              {t("adminPages.tickets.emptyTitle", { defaultValue: "No tickets found" })}
            </p>
            <p className="max-w-sm text-xs text-slate-500">
              {t("adminPages.tickets.emptyDesc", { defaultValue: "Support requests will appear here when students or instructors submit them." })}
            </p>
          </div>
        ) : null}
              ...
              <table className="w-full min-w-[980px]">
                <thead className="bg-slate-50 dark:bg-white/5">
                  <tr className="text-start">
                    {["id", "subject", "from", "category", "priority", "status", "date", "actions"].map((h) => (
                      <th key={h} className="px-4 py-3 text-start text-xs font-bold uppercase tracking-widest text-slate-500">
                        {t(`adminPages.tickets.table.${h}`, { defaultValue: h })}
                      </th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="px-4 py-12 text-center text-sm text-slate-500">
                        {t("adminPages.tickets.noMatch", { defaultValue: "No tickets match your filters." })}
                      </td>
                    </tr>
                  ) : (
                    filtered.map((ticket) => (
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 opacity-70 transition group-hover:opacity-100">
                            <Link
                              ...
                              title={t("adminPages.tickets.tooltipView", { defaultValue: "View Ticket" })}
                            >
                            ...
                            <button
                              ...
                              title={t("adminPages.tickets.tooltipResolve", { defaultValue: "Resolve Ticket" })}
                            >
