...
export default function UserDetail() {
  ...
  if (isError || !user) {
    return (
      <section>
        <PageHeader title={t("adminPages.userDetail.title")} subtitle="" />
        <div className="rounded-2xl border border-red-100 bg-yu-blue-100 p-6 text-sm text-red-800">
          {t("adminPages.userDetail.loadError")}
        </div>
        <Link to="/admin/users" className="mt-4 inline-flex text-sm font-semibold text-yu-blue-700 hover:underline">
  ...
  return (
    <section>
      <Link
        to="/admin/users"
        className="mb-6 inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-yu-blue-700"
      >
        ...
        {tabs.map((item) => (
          <button
            ...
            className={`border-b-2 px-4 py-2 text-sm font-semibold ${
              tab === item.key
                ? "border-yu-blue-700 text-yu-blue-700"
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
            <div className="flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-slate-500">
              <User className="h-4 w-4 text-yu-blue-700" />
          ...
          <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-yu-blue-700/10 to-white p-6 shadow-sm">
            <div className="flex items-center gap-2 text-sm font-bold text-slate-900">
              <Shield className="h-4 w-4 text-yu-blue-700" />
              {t("adminPages.userDetail.quickLinks", "Quick links")}
            </div>
            <div className="mt-4 flex flex-col gap-2">
              {role === ROLES_STUDENT ? (
                <Link to={`/admin/students/${user.id}`} className="rounded-xl bg-yu-blue-700 px-4 py-2.5 text-center text-sm font-semibold text-white">
                  {t("adminPages.userDetail.openStudent")}
                </Link>
              ) : null}
              {role === ROLES_INSTRUCTOR ? (
                <Link to={`/admin/instructors/${user.id}`} className="rounded-xl bg-yu-blue-700 px-4 py-2.5 text-center text-sm font-semibold text-white">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                ...
                <input
                  ...
                  className="rounded-lg border px-3 py-2 text-sm"
                />
                <button type="button" onClick={() => void handleGrant()} className="rounded-lg bg-yu-blue-700 px-4 py-2 text-sm font-semibold text-white">
