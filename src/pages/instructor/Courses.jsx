...
import { 
  Plus, Search, FileText, Clock, Users, Trash2, Edit3, Eye, AlertCircle, BookOpen, Loader2
} from "lucide-react";
...
const DARK_INPUT =
  "h-10 w-full rounded-xl border border-slate-200 bg-white ps-10 pe-4 text-xs text-slate-900 outline-none focus:border-[var(--yu-blue-700)] transition-all dark:border-white/15 dark:bg-[#12121a] dark:text-slate-100 dark:[color-scheme:dark]";
    ...
    <div className="space-y-6 antialiased font-sans pb-20">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {dir === "rtl" 
              ? "صمم محتوى Step 1، وأدر الواجبات والاختبارات وبنوك الأسئلة الطبية." 
              : "Author Step 1 curricula, assign medical reviews, and build board-style question banks."}
          </p>
        </div>

        <button
          onClick={() => navigate("/instructor/courses/new")}
          className="flex items-center justify-center gap-2 rounded-xl bg-[var(--yu-blue-700)] hover:bg-[var(--yu-blue-600)] text-white px-5 py-2.5 text-xs font-bold shadow-lg shadow-[var(--yu-blue-700)]/15 transition-all self-start md:self-auto"
        >
      ...
      {isLoading ? (
        <div className="flex py-20 items-center justify-center">
          <Loader2 className="animate-spin h-8 w-8 text-[var(--yu-blue-700)]" />
        </div>
        ...
        <div className={`rounded-3xl border-2 border-dashed border-slate-200 p-12 text-center bg-white ${DARK_SURFACE} dark:border-white/10`}>
          ...
          <p className="text-xs text-slate-500 mt-2">
            {dir === "rtl" 
              ...
              : "Create your first USMLE Step 1 course today."}
          </p>
        </div>
      ) : (
          ...
          {coursesList.map(c => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] font-semibold text-[var(--yu-blue-700)] tracking-wider mb-2 uppercase">
                  {c.category?.name || (dir === "rtl" ? "أنظمة الأعضاء" : "Organ Systems")}
                </p>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 border-t border-slate-100 dark:border-white/5 pt-3 text-center">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-[var(--yu-blue-700)] flex items-center justify-center gap-1 mt-0.5">
                    {Number(c.price || 0).toLocaleString()} USD
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4 flex flex-col gap-2">
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-50 hover:bg-slate-100 dark:bg-white/5 dark:hover:bg-white/10 text-xs font-bold text-[var(--yu-blue-700)] py-2.5 transition-all"
                >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Link
                    ...
                    className="col-span-2 flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 dark:border-white/10 dark:text-slate-300 dark:hover:bg-white/5"
                  >
