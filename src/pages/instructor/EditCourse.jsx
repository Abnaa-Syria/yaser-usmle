...
const DARK_FORM_TEXTAREA =
  "w-full rounded-xl border border-slate-200 bg-white p-3 text-xs text-slate-900 outline-none transition-all placeholder:text-slate-400 focus:border-[var(--yu-blue-700)] focus:ring-2 focus:ring-[var(--yu-blue-700)]/20 dark:border-white/15 dark:bg-[#12121a] dark:text-slate-100 dark:placeholder:text-slate-500";
const DARK_FORM_TEXTAREA_LG =
  "w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs font-semibold text-slate-900 outline-none transition-all placeholder:text-slate-400 focus:border-[var(--yu-blue-700)] focus:ring-2 focus:ring-[var(--yu-blue-700)]/20 dark:border-white/15 dark:bg-[#12121a] dark:text-slate-100 dark:placeholder:text-slate-500";
...
function EditCourse() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    return ["details", "curriculum", "exams"].includes(tab) ? tab : "details";
  });
  ...
  useEffect(() => {
    const tab = searchParams.get("tab");
    if (tab && ["details", "curriculum", "exams"].includes(tab)) {
      setActiveTab(tab);
    }
  }, [searchParams]);
    ...
    return (
      <div className="flex h-[60vh] items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <Loader2 className="h-10 w-10 animate-spin text-[var(--yu-blue-700)]" />
          <p className="text-xs font-bold text-slate-400">
            {dir === "rtl" ? "جاري تحميل تفاصيل الكورس..." : "Loading course details..."}
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="text-center py-20 bg-white dark:bg-[#1E293B] rounded-3xl border border-slate-200 dark:border-white/5">
        ...
        <p className="text-xs text-slate-500 mt-2">
          {dir === "rtl" ? "عذراً، لم نتمكن من العثور على الكورس المطلوب أو لا تملك صلاحية الوصول إليه." : "We couldn't locate this course, or you do not have permission to manage it."}
        </p>
        <Link to="/instructor/courses" className="mt-4 inline-block text-xs font-bold text-[var(--yu-blue-700)] hover:underline">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 dark:border-white/10 pb-5">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <Link to="/instructor/courses" className="text-slate-400 hover:text-[var(--yu-blue-700)] transition-colors">
          ...
          <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider ps-7">
            {course.category?.name || (dir === "rtl" ? "أنظمة الأعضاء" : "Organ Systems")}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 ps-7 sm:ps-0">
          ...
          {(course.status === "DRAFT" || course.status === "REJECTED") && (
            <button
              ...
              className="flex items-center justify-center gap-1.5 rounded-xl bg-[var(--yu-blue-700)] hover:bg-[var(--yu-blue-600)] text-white px-4 py-2 text-xs font-bold shadow-md shadow-[var(--yu-blue-700)]/15 transition-all"
            >
        ...
        {[
          ...
          <button
            ...
            className={`py-3.5 px-5 text-xs font-bold relative transition-all border-b-2 font-cairo ${
              activeTab === tab.id
                ? "border-[var(--yu-blue-700)] text-[var(--yu-blue-700)]"
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-xl">
        ...
        <div className="border-t border-slate-100 dark:border-white/5 pt-4">
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              className="h-4 w-4 rounded border-slate-300 text-[var(--yu-blue-700)] focus:ring-[var(--yu-blue-700)]"
            />
          ...
          <div className="space-y-3">
            {(form.pricingTiers || []).map((tier, idx) => (
              <div key={idx} className="grid grid-cols-1 md:grid-cols-5 gap-2 p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/40 relative">
                ...
                <label className="block space-y-1 md:col-span-1">
                  <span className="text-[9px] font-bold text-slate-400">{dir === "rtl" ? "السعر (USD)" : "Price (USD)"}</span>
                  <input
                    type="number"
                    min={0}
                    value={tier.price}
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 rounded-xl bg-[var(--yu-blue-700)] hover:bg-[var(--yu-blue-600)] text-white py-3 text-xs font-bold shadow-md shadow-[var(--yu-blue-700)]/15 transition-all"
      >
...
function CurriculumTab({ 
  ...
  const handleAddUnit = async () => {
    try {
      ...
      const title = dir === "rtl" ? `الوحدة ${order}: عنوان الوحدة الجديدة` : `Unit ${order}: New Step 1 Unit`;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 font-cairo">
          {dir === "rtl" ? "هيكل منهج الكورس" : "Course Curriculum Structure"}
        </h3>
        <button
          onClick={handleAddUnit}
          className="flex items-center gap-1.5 rounded-lg bg-[var(--yu-blue-700)]/10 hover:bg-[var(--yu-blue-700)]/20 text-[var(--yu-blue-700)] px-3.5 py-2 text-xs font-bold transition-all"
        >
                ...
                <div className="flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/40 p-4 border-b border-slate-100 dark:border-white/5">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="p-1.5 text-[var(--yu-blue-700)] hover:bg-[var(--yu-blue-700)]/10 rounded-lg shrink-0"
                    >
                      <Plus className="h-4 w-4" />
                    </button>
                    <button
                            ...
                            <div key={ex.id} className="flex items-center justify-between bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/10 dark:border-amber-500/20 px-3.5 py-2 rounded-xl">
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  ...
                                  className="text-[10px] font-extrabold text-[var(--yu-blue-700)] hover:underline"
                                >
...
function LessonRow({ lesson, lessonExams, onAddQuiz, onEditExam, onDeleteExam, onBlurTitle, onUpdateSpecs, onDelete, dir }) {
  ...
  const [localVdoCipherVideoId, setLocalVdoCipherVideoId] = useState(lesson.vdoCipherVideoId || "");
  ...
  useEffect(() => {
    setLocalVdoCipherVideoId(lesson.vdoCipherVideoId || "");
  ...
  const handleSave = () => {
    const specs = {
      vdoCipherVideoId: localVdoCipherVideoId.trim() || null,
  ...
  const isSetup = !!lesson.vdoCipherVideoId || !!lesson.videoUrl || !!localVdoCipherVideoId || !!localVideoUrl;
  const badgeText = isSetup
    ? (dir === "rtl" ? "Pre-recorded" : "Pre-recorded")
    : (dir === "rtl" ? "+ Video" : "+ Video");
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            className={`px-2 py-1 rounded-lg text-[9px] font-extrabold border transition-colors ${
              isSetup
                ? "bg-[var(--yu-blue-700)]/10 text-[var(--yu-blue-700)] border-[var(--yu-blue-700)]/20"
          ...
          <button
            ...
            className="p-1.5 text-[var(--yu-blue-700)] hover:bg-[var(--yu-blue-700)]/10 rounded-lg"
          >
          ...
          {lessonExams.map((ex) => (
            <div key={ex.id} className="flex items-center justify-between bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/10 dark:border-amber-500/20 px-2.5 py-1.5 rounded-lg text-[10px]">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <button
                  onClick={() => onEditExam(ex.id)}
                  className="font-extrabold text-[var(--yu-blue-700)] hover:underline"
                >
                  {dir === "rtl" ? "تعديل الأسئلة" : "Edit Qs"}
                </button>
        ...
        <div className="border-t border-slate-100 dark:border-slate-800 pt-3 mt-2 space-y-3 animate-in fade-in slide-in-from-top-1 duration-150">
          <div className="grid gap-2">
            <div>
              <label className={DARK_FORM_LABEL}>
                {dir === "rtl" ? "VdoCipher Video ID" : "VdoCipher Video ID"}
              </label>
              <input
                type="text"
                value={localVdoCipherVideoId}
                onChange={(e) => setLocalVdoCipherVideoId(e.target.value)}
            ...
            <div>
              <label className={DARK_FORM_LABEL}>
                {dir === "rtl" ? "رابط فيديو قديم (اختياري)" : "Legacy Video URL (optional)"}
              </label>
              <input
                type="url"
                value={localVideoUrl}
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-white/5">
            ...
            <button
              type="button"
              onClick={handleSave}
              className="px-3 py-1.5 rounded-lg bg-[var(--yu-blue-700)] hover:bg-[var(--yu-blue-600)] text-white text-[10px] font-bold"
            >
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg bg-[var(--yu-blue-700)] hover:bg-[var(--yu-blue-600)] text-white px-3.5 py-2 text-xs font-bold shadow-md shadow-[var(--yu-blue-700)]/15 transition-all shrink-0"
        >
          <Plus className="h-4 w-4" />
            ...
            >
              ...
              <div className="flex items-center gap-2 self-end sm:self-auto">
                <Link
                  ...
                  className="inline-flex items-center gap-1 rounded-lg bg-[var(--yu-blue-700)]/10 hover:bg-[var(--yu-blue-700)]/20 text-[var(--yu-blue-700)] px-3.5 py-1.5 text-xs font-bold"
                >
