...
function Settings() {
  ...
  if (isError || !profile) {
    return (
      <section className="rounded-xl border border-red-200 bg-[var(--yu-blue-700)]/10 p-4 text-sm text-red-700 dark:border-red-500/30 dark:bg-[var(--yu-blue-700)]/10 dark:text-red-300">
        {getErrorMessage(error, t("dashboard.instructor.pages.settings.loadError"))}
        <button
          type="button"
          onClick={() => refetch()}
          className="ms-3 rounded-lg bg-[var(--yu-blue-700)] px-3 py-1 text-xs font-bold text-white"
        >
